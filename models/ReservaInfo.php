...
class VueloInfo extends ActiveRecord{
    ...
    public function consultarRegistros($id) {
        return "SELECT 
                ...
                FROM 
                    vuelo
                JOIN 
                    avion ON vuelo.avionId = avion.id        
                JOIN 
                    aeropuerto ON vuelo.aeropuertoOrigenId = aeropuerto.id
                JOIN 
                    aeropuerto AS aeropuerto2 ON vuelo.aeropuertoDestinoId = aeropuerto2.id
                JOIN 
                    horario ON vuelo.fechaSalida = horario.id
                JOIN 
                    horario AS horario2 ON vuelo.fechaLlegada = horario2.id
                JOIN 
                    tarifa ON vuelo.tarifaId = tarifa.id
                WHERE
                    vuelo.id = $id
                GROUP BY 
                    vuelo.id";
    ...
    public function buscarVueloPorId($id) {
        return $this->generarConsultaBase() . " WHERE vuelo.id = '$id'";
    }
}